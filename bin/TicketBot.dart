...
Future<void> main(List<String> arguments) async {
  ...
  bot.onMessageReactionAdded.listen((event) async {
    if (event.messageId == Snowflake(861272991894994955)) {
      ...
      var channel = await guild.createChannel(
              ChannelBuilder('ticket-${user.username}', ChannelType.text))
          as GuildChannel;
  ...
  interact
    ..onMultiselectEvent.listen((event) async {
      if (event.interaction.customId == 'select') {
        ...
        as TextGuildChannel;
        await chan.edit(name: '$choice-${chan.name.split('-')[1]}');
        switch (choice) {
          case 'support':
        ...
        var name = chan.permissionOverrides
            ...
            .id;
        chan.permissionOverrides.removeWhere(
    ...
    ..registerSlashCommand(SlashCommandBuilder(
        'close', 'Close the ticket channel', [],
    ...
    ..onSlashCommand.listen((event) async {
      ...
      if (event.interaction.name == 'close') {
        var chanName = guild.channels
            .firstWhere((element) => element.id == chan.id)
            .name;
